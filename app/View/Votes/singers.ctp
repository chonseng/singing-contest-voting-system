...
<style>
	...
	<div class="row">
		<div class="header">
			<?php if ($header=="") :?>
			<h1 id="header_text">你需投<?=$_SESSION["voting_amount"]?>票</h1>
			<?php else : ?>
			<h1 id="header_text"><?=$header?></h1>
			<?php endif; ?>
	...
	<div class="row">
		<form action="<?php $this->Path->myroot(); ?>votes/add" method="post" id="singerform">
				...
				<div id="singer_<?= $singer['Singer']['id']?>" class="singer">
					<h2 class="singer_name"><?=$singer['Singer']['name']?></h2>
					<div class="singer_photo" style="background-image: url(../img/singers/<?= $singer['Singer']['singer_id']?>.jpg)"></div>
					<!-- <img src="<?php $this->Path->myroot(); ?>img/singers/<?= $singer['Singer']['id']?>.jpg" alt=""> -->
					<div class="radio-toolbar">
						<input class="mycheckbox" type="checkbox" name="Vote[singer_id][<?=$singer['Singer']['singer_id']?>]" value="<?=$singer['Singer']['singer_id']?>" id="<?=$key?>">
